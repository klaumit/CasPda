...
T_ICON TiconNew = { &TchHeaderIcon[0], NULL, NULL, 0x00 };
T_ICON TiconLevel = { &TchHeaderIcon[1], NULL, NULL, 0x00 };
T_ICON TiconHigh = { &TchHeaderIcon[2], NULL, NULL, 0x02 };


TCHTBL TchHeaderIcon2[] = {
	99, 0, 127, 11,
	ACT_MAKE | ACT_MOVE_IN | ACT_MOVE_OUT | ACT_BREAK_IN,
	OBJ_IC_SET,
	0x0000,
...
T_ICON TiconSet = { &TchHeaderIcon2[0], NULL, NULL, 0x00 };
T_ICON TiconClr = { &TchHeaderIcon2[1], NULL, NULL, 0x00 };
...
T_ICON TiconHOk = { &TchDispHigh[0], NULL, NULL, 0x02 };
T_ICON TiconHClr = { &TchDispHigh[1], NULL, NULL, 0x02 };


TCHTBL TchGameArea1[] = {     /* GameArea Level1 */
...
TCHTBL TchGameArea2[] = {     /* GameArea Level2 */
...
TCHTBL TchGameArea3[] = {     /* GameArea Level3 */
...
static byte LevelSet(void)
{
	...
	/* data */
	strcpy((char*)t_tbl[0],"Level1");
	strcpy((char*)t_tbl[1],"Level2");
	strcpy((char*)t_tbl[2],"Level3");
...
static void DspFileOpenErr(void){
	...
	LibPutProStr((byte)IB_PFONT1,30,35,(byte *)"PV aplication memory",110);
	LibPutProStr((byte)IB_PFONT1,30,44,(byte *)"is full!",110);
	LibPutProStr((byte)IB_PFONT1,30,53,(byte *)"See your User's Guide",110);
	LibPutProStr((byte)IB_PFONT1,30,62,(byte *)"for more infomation.",110);
...
void PuzzleDisplay(void){
	...
	LibPutProStr((byte)IB_PFONT1,104,2,(byte *)"New",30);				/* New */
	LibPutProStr((byte)IB_PFONT1,74,2,(byte *)"Level",30);			/* Level */
	LibPutProStr((byte)IB_PFONT1,108,15,(byte *)"High score",50);		/* High score */
...
//	char t[6];
	...
	if(HighScoreCk()==TRUE){
		...
		LibPutFarData(71,0,8);
		LibPutProStr((byte)IB_PFONT1,79,2,(byte *)"Clr",18);				/* Clr */
		LibPutProStr((byte)IB_PFONT1,106,2,(byte *)"Set",18);				/* Set */
		LibPutFarData((LCD_X_DOT-128)/2,27,107);
		LibPutFarData(75,58,135);

		LibPutProStr((byte)IB_PFONT3,33,33,(byte *)"Congratulations!",110);	/* Congratulations! */
		LibPutProStr((byte)IB_PFONT3,26,46,(byte *)"Time",50);				/* Time */
		LibPutProStr((byte)IB_PFONT3,26,59,(byte *)"Name",50);				/* Name */

		Num2( &t[0], puzzleinfo.timer/120);
		t[2]=':';
		Num2( &t[3], (puzzleinfo.timer%120)/2);
		t[5]='\0';
		LibPutProStr((byte)IB_PFONT3,97,46,t,34);
		...
		LibPutProStr((byte)IB_PFONT3,33,38,(byte *)"Congratulations!",110);			/* Congratulations! */
		LibPutProStr((byte)IB_PFONT3,((LCD_X_DOT-128)/2)+10,56,(byte *)"Time",50);	/* Time */
...
static void HighScoreSave(byte *name){
	...
	if(gamelevel == 0){
		strcpy((char*)sc->lv1,(char*)name);
		sc->lv1time = puzzleinfo.timer;
	}else if(gamelevel == 1){
		strcpy((char*)sc->lv2,(char*)name);
		sc->lv2time = puzzleinfo.timer;
	}else{
		strcpy((char*)sc->lv3,(char*)name);
...
static bool HighScoreCk(void){
	...
	if(time == 0xffffffff || time > puzzleinfo.timer)
		return(TRUE);
...
static void HighScoreClr(void){
	...
	if(sc->lv1time == 0xffffffff && sc->lv2time == 0xffffffff && sc->lv3time == 0xffffffff)
		return;
...
static void DispHSData(void){
	...
	LibPutProStr((byte)IB_PFONT2,DSP_HS_X+48,DSP_HS_Y+4,(byte *)"Level1",50);			/* Level1 */
	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+13,(byte *)"Time",30);			/* Time */
	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+25,(byte *)"Name",30);			/* Name */

	LibPutProStr((byte)IB_PFONT2,DSP_HS_X+48,DSP_HS_Y+39,(byte *)"Level2",50);		/* Level2 */
	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+48,(byte *)"Time",30);			/* Time */
	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+60,(byte *)"Name",30);			/* Name */

	LibPutProStr((byte)IB_PFONT2,DSP_HS_X+48,DSP_HS_Y+74,(byte *)"Level3",50);		/* Level3 */
	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+83,(byte *)"Time",30);			/* Time */
	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+9,DSP_HS_Y+95,(byte *)"Name",30);			/* Name */

	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+17,DSP_HS_Y+112,(byte *)"Clr",30);			/* Clr */
	LibPutProStr((byte)IB_PFONT1,DSP_HS_X+99,DSP_HS_Y+112,(byte *)"OK",30);			/* OK */

	if(sc->lv1time != 0xffffffff){
		TimeChg(t,sc->lv1time);
		LibPutProStr((byte)IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+13,t,34);
		LibPutProStr((byte)IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+25,sc->lv1,58);
	}

	if(sc->lv2time != 0xffffffff){
		TimeChg(t,sc->lv2time);
		LibPutProStr((byte)IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+48,t,34);
		LibPutProStr((byte)IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+60,sc->lv2,58);
	}

	if(sc->lv3time != 0xffffffff){
		TimeChg(t,sc->lv3time);
		LibPutProStr((byte)IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+83,t,34);
		LibPutProStr((byte)IB_PFONT1,DSP_HS_X+60,DSP_HS_Y+95,sc->lv3,58);
	}
	LibPutDisp();
}
